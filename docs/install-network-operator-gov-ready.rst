...
- A namespace to deploy the NVIDIA Network Operator.
  The example install commands below use ``network-operator`` as the namespace.
...
.. code-block:: console
   ...
   $ kubectl create secret -n network-operator docker-registry ngc-secret \
...
#. Label your ``network-operator`` namespace for the Operator to set the enforcement policy to privilege.
...
This secret name is set as value of ``ofedDriver.env`` and ``UBUNTU_PRO_TOKEN`` environment variable of the Network Operator NicClusterPolicy.
